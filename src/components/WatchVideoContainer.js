...
const getVideoInfo= async()=>{
  ...
  return (
    ...
    <div className='mx-1 sm:mx-7 mb-5 max-w-lg sm:max-w-5xl '>
      <iframe src={"https://www.youtube.com/embed/" + searchParams.get("v") } frame-border="0" allow="accelerometer; autoPlay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen className='rounded-xl w-[150%] h-[390px] sm:w-[1000px] sm:h-[555px]'></iframe>
      <h1 className='m-2 font-bold w-[150%] sm:w-[1000px]'>{snippet?.title}</h1>
      <div className='flex justify-between   w-[150%] sm:w-[1000px]'>
      <div className='flex m-2'>
        ...
        <div className='flex p-1 h-12 m-1 '>
          ...
          <div id='watch-button' className='flex mr-1 '>
          <p ><AiOutlineShareAlt size={18} className=' mx-1'/> Share</p>
          <p ><AiOutlineDownload size={18} className=' mx-1'/> Download</p>
          <p ><AiOutlineSave size={18} className=' mx-1'/> Save</p></div>
          
        </div>
    </div>
    <div className={`relative bg-gray-200 p-2 rounded-lg ${isOverflowHidden? 'h-16 p-1 overflow-hidden   w-[145%] sm:w-[1000px]' : '  w-[145%] sm:w-[1000px]'} text-sm ml-3 sm:ml-0 mr-5 -z-10 `} onClick={()=> handleDivClick()}>
